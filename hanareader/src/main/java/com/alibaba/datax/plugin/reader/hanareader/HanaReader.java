...
public class HanaReader extends Reader {
    ...
    public static class Job extends Reader.Job {
        private static final Logger LOG = LoggerFactory.getLogger(HanaReader.Job.class);
        ...
        private void dealFetchSize(Configuration originalConfig) {
            int fetchSize = originalConfig.getInt(com.alibaba.datax.plugin.rdbms.reader.Constant.FETCH_SIZE, Constant.DEFAULT_FETCH_SIZE);
            if (fetchSize < 1) {
                throw DataXException.asDataXException(DBUtilErrorCode.REQUIRED_VALUE, String.format("您配置的 fetchSize 有误，fetchSize:[%d] 值不能小于 1.", fetchSize));
            }
        ...
        private void dealHint(Configuration originalConfig) {
            ...
            if (StringUtils.isNotBlank(hint)) {
                ...
                if (!isTableMode) {
                    throw DataXException.asDataXException(HanaReaderErrorCode.HINT_ERROR, "当且仅当非 querySql 模式读取 hana 时才能配置 HINT.");
                }
